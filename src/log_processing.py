...
def all_logs(log_file):
    ...
    for log_ in all_logs:
        ...
        logs_with_date.append((log_date, log_level, log_content))
...
def last_logs(log_file):
    ...
    return [
        (date_, level_, log_)
        for date_, level_, log_ in all_logs_
        if date_ == all_logs_[-1][0]
    ]
...
def log_content_styled(log):
    highlight_log = ''.join(
        [
            apply_warning_highlight(level_, content_)
            for _, level_, content_ in log
        ]
    )
...
def mime_text(body_title, body_content):
    font_family = 'Geneva,' + ' Verdana,' + ' sans-serif;'
